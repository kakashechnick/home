...
def comparison_of_lengths(arr: list):
  ...
  for item in arr: #Окончательный проход
    if len(item) == len(maxlen): #Если в списке есть объект такой же по длине как и наибольший, то
      ...
      start = arr.index(item) + 1 #Метод index выводит первый подходящий по запросу, поэтому берется последний найденный элемент(его индекс) и делаеться исключение, по началу входа
...
class Users:
  ...
  def adduser(self, lastname, district, address, phone, contract_number, date_of_the_agreement, price, data_last_payment): #добавление элементов в список
    instance = {
      'фамилия': lastname,
      'район': district,
      'адрес': address,
    ...
    while check: #Потребность в сортироваке
      ...
      for item in range(len(self._users) - 1): #Берется вся длина списка, по которой будет происходлить сортеровка
        if self._users[item][oper] > self._users[item + 1][oper]: #Берутся 2 рядостоящих элемента
          self._users[item], self._users[item + 1] = self._users[item + 1], self._users[item] #Если левый элемент больше чем правый, они меняються местами
...
if __name__ == '__main__':
  ...
  for i in range(10):
    user.adduser(
      ...
      f"{datetime.date(random.randint(2020, 2021), random.randint(1, 12), random.randint(1, 30))}", #ValueError: day is out of range for month
      f'{random.randrange(5000, 10000, 500)} Pуб.',
      f"{datetime.date(random.randint(2022, 2023), random.randint(1, 12), random.randint(1, 30))}" #ValueError: day is out of range for month
    )
  print('\n')
